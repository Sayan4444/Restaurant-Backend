...
const restaurantDataHandler = async () => {
    const locations = await Location.find();
    const cuisines = await Cuisine.find();

    const indianCuisineId = cuisines.find(
        (cuisine) => cuisine.name === "indian"
    )?.id;
    const mexicanCuisineId = cuisines.find(
        (cuisine) => cuisine.name === "mexican"
    )?.id;
    const italianCuisineId = cuisines.find(
        (cuisine) => cuisine.name === "italian"
    )?.id;

    const ottawaLocationId = locations.find(
        (location) => location.name === "ottawa"
    )?.id;
    const torontoLocationId = locations.find(
        (location) => location.name === "toronto"
    )?.id;
    const niagaraLocationId = locations.find(
        (location) => location.name === "niagara"
    )?.id;
...
const reviewDataHandler = async () => {
    ...
    const userLaith = await User.findOne({
        email: 'laith@hotmail.com',
    }).select('id');
    const userJosh = await User.findOne({
        email: 'josh@hotmail.com',
    }).select('id');
    const userLebron = await User.findOne({
        email: 'lebron@hotmail.com',
    }).select('id');
    const userCassidy = await User.findOne({
        email: 'cassidy@hotmail.com',
    }).select('id');
...
module.exports = { locationData, cuisineData, restaurantDataHandler, itemDataHandler, userData, reviewDataHandler }